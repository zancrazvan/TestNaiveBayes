 ...
 */
package com.varm.classifiers;

import com.varm.dataobjects.Document;
import com.varm.dataobjects.FeatureStats;
import com.varm.dataobjects.NaiveBayesKnowledgeBase;
import com.varm.features.FeatureExtraction;
import com.varm.features.TextTokenizer;
 ...
 * @see <a href=
 *      "http://blog.datumbox.com/developing-a-naive-bayes-text-classifier-in-java/">http://blog.datumbox.com/developing-a-naive-bayes-text-classifier-in-java/</a>
 */
public class NaiveBayes {
	...
	private FeatureStats selectFeatures(List<Document> dataset) {
		...
		// System.out.println("Dupa apel Chisquare: " +
	 ...
	 */
	public void train(Map<String, String[]> trainingDataset, Map<String, Double> categoryPriors)
			throws IllegalArgumentException {
		...
		knowledgeBase.d = featureStats.featureCategoryJointCount.size(); // number
																			// of
																			// features
		System.out.println("Nr. of fetarures: " + knowledgeBase.d);
		...
		if (categoryPriors == null) {
			...
			for (Map.Entry<String, Integer> entry : featureStats.categoryCounts.entrySet()) {
				...
				System.out.println("Nr of Observations: " + knowledgeBase.n);
				System.out.println("LogPriorsTemp: " + knowledgeBase.logPriors.toString());
			}
		} else {
		...
		for (String category : knowledgeBase.logPriors.keySet()) {
			for (Map.Entry<String, Map<String, Integer>> entry : featureStats.featureCategoryJointCount.entrySet()) {
				...
				logLikelihood = Math
						.log((count + 1.0) / (featureOccurrencesInCategory.get(category) + knowledgeBase.d));
	...
	public String toString() {
		return "NaiveBayes [chisquareCriticalValue=" + chisquareCriticalValue + ", knowledgeBase=" + knowledgeBase
				+ "]";
	}
